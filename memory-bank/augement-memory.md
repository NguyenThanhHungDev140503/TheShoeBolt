...
- User prefers comprehensive Memory Bank updates that include reading all core files, analyzing current codebase state, comparing with documented state, and updating files to maintain accuracy - especially activeContext.md, progress.md, systemPatterns.md, and techContext.md.
- User wants to migrate the project from Node.js runtime to Deno, indicating a preference for Deno's features like native TypeScript support and security model.
